...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.loss_type == "iou":
            ...
            loss = 1 - ciou_loss(pred, target).clamp(min=-1.0, max=1.0)
...
def ciou_loss(pred, target):
    ...
    tc = (torch.atan(pred[:, 2] / pred[:, 3]) - torch.atan(target[:, 2] / target[:, 3])) * 2 / math.pi
...
# def ciou_loss(pred, target, lambda_scale=0.5):
#     assert pred.shape[0] == target.shape[0]
...
#     # 计算尺度损失
#     delta_w = torch.abs(pred[:, 2] - target[:, 2]) / target[:, 2]
#     delta_h = torch.abs(pred[:, 3] - target[:, 3]) / target[:, 3]
#     scale_loss = lambda_scale * (delta_w + delta_h)

#     # 总损失是CIoU损失和尺度损失的和
#     total_loss = ciou + scale_loss
